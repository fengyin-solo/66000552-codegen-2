...
import type { Tick, Fill, OrderBook, GridConfig, GridResult, GridBounds } from '@/types'
export const useTradingStore = defineStore('trading', () => {
  ...
  const gridBounds = ref<GridBounds>({ lowerPrice: 95, upperPrice: 115, gridCount: 20 })
  ...
  function connectWS() {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (d.ticks) ticks.value = d.ticks
        if (d.fills) fills.value = d.fills
        if (d.grid) gridBounds.value = d.grid
